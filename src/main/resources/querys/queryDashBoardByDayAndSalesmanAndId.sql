...
select slm.name as row_nam,extract (dow from saled_at) as day, sum(amount_sale) as value from (select * from salesman s  where s.id in (1,2)) as slm,
...
select * from crosstab('select slm.name as row_nam,extract (dow from saled_at) as day, sum(amount_sale) as value from (select * from salesman s  where s.id in (1,2)) as slm,
(select sales_man_id,saled_at,amount_sale from sale where sales_man_id  in (1,2)  and saled_at >= ''2020-03-29'' and saled_at  <= ''2020-04-05'') as sl
group by saled_at,slm.name order by name,day',
